...
export const tr = {
  ...
  game: {
    ...
    streak: 'Seri',
    streakDays: (n: number) => (n === 1 ? '1 gün' : `${n} gün`),
    weekly: 'Bu hafta',
    badges: 'Rozetler',
    badgesEarnedCount: (earned: number, total: number) => `${earned}/${total} rozet`,
    earned: 'Kazanıldı',
    locked: 'Henüz kazanılmadı',
    tasksDone: (n: number) => (n === 1 ? '1 görev' : `${n} görev`),
    emptyTitle: 'Henüz puan yok',
    emptyBody: 'Görev tamamladıkça puanlar, seriler ve rozetler burada birikir.',
    yourStanding: 'Senin durumun',
    celebratePoints: (n: number) => `+${n} puan`,
    celebrateStreak: (n: number) => `${n} günlük seri`,
    newBadge: 'Yeni rozet',
    niceWork: 'Eline sağlık',
  },
